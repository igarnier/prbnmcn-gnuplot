...
module GP_script : sig
  ...
  let make_error_bars (data : r2 Seq.t) (error_bars : r2 Seq.t) =
    seq_map2 data error_bars (fun ((x, y) : r2) ((y1, y2) : r2) ->
        r4 x y (y +. y1) (y -. y2))
...
let get_targets ?(path = "gnuplot") () =
  let ic =
    Unix.open_process_args_in
      path
      [| path; "-e"; "set print \"-\"; print(GPVAL_TERMINALS); quit" |]
  in
  ...
  (try
   ...
   with End_of_file -> ()) ;
  let process_status = Unix.close_process_in ic in
  match process_status with
  | Unix.WEXITED 0 ->
      let file = Buffer.contents buf in
      Option.some (String.split_on_char ' ' (String.trim file))
  | _ -> failwith "get_targets: call to gnuplot failed"
...
module GP_run = struct
  ...
  let run_script ?(path = "gnuplot") ?(detach = false) ~target
      ~(plot : GP_script.t) () =
    let is_interactive = is_target_interactive target in
    let block_interactive = (not detach) && is_interactive in
    let (name, oc) = Filename.open_temp_file ~perms:0o666 "gnuplot" ".gp" in
    let full_command =
      concat
        ([set_target target; (plot :> string)]
        @ if block_interactive then ["pause mouse close"] else [])
    in
    output_string oc full_command ;
    close_out oc ;
    let args =
      if detach && is_interactive then [path; "--persist"; name]
      else [path; name]
    in
    let (ic, oc) = Unix.open_process_args path (Array.of_list args) in
    let ps = Unix.close_process (ic, oc) in
    Unix.unlink name ;
    match ps with
    | WEXITED 0 -> ()
    | _ ->
  ...
  let write_and_run_script ?path ?detach ~filename ~target ~(plot : GP_script.t)
      () =
    write_script ~filename ~target ~plot ;
    run_script ?path ?detach ~target ~plot ()
end
