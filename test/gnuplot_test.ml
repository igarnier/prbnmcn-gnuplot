          ...
          (if recent_enough_version then
             Style.(
               ...
               |> set_circle
                    ~fill:
                      Fill.(
                        default |> set_solid ~density:0.35 ~transparent:true)
                    ~radius:0.2)
           else Style.default)
        ~mu:(10., 3.)
        rng_state ]
...
let () =
  Plot.(
    run_matrix
      ~target:(Plot.png ~pixel_size:(1280, 960) ~png_file:"figure.png" ())
...
let () =
  Plot.(
    run_matrix
      ~target:(png ~pixel_size:(1280, 960) ~png_file:"scatter.png" ())
      ~title:"meta-title"
      exec_detach
      [| [| Some scatter_2d; Some scatter_3d |] |])
...
let () =
  Plot.(
    run_matrix
      ~target:(png ~pixel_size:(1280, 960) ~png_file:"hist.png" ())
      ~title:"meta-title"
      exec_detach
      [| [| Some histogram; Some lines |] |])
...
let () =
  Plot.(
    run_matrix
      ~target:(png ~pixel_size:(1280, 960) ~png_file:"error_bars.png" ())
      ~title:"meta-title"
      exec
      [| [| Some error_bars; Some error_lines |] |])
